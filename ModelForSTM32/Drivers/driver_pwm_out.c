...
void SetPwm(int16_t set_pwm)
{
    ...
    if(PWM_Mode==0)//   
    {
        ...
        /* 配置数据按比例和起始值赋值寄存器 */
//        TIM3->CCR2 = ( pwm_tem[CH_out_Mapping[0]] ) * PWM_RADIO + INIT_DUTY; //pwmout1
        TIM3->CCR2 = 0 * PWM_RADIO + INIT_DUTY; //pwmout1
    ...
    {
        /* 配置数据按比例和起始值赋值寄存器 */
        TIM3->CCR2 = 1000 * PWM_RADIO + INIT_DUTY; //pwmout1
    }
}
